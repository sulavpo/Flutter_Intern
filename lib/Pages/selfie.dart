...
import 'package:mohar_app/models/selfie_page_view.dart';
...
class _SelfieClickState extends State<SelfieClick> {
  ...
  void _doSomething() async {
    if (pickedImage3 == null) {
      return Toasts.showToast("Please take a selfie", Colors.red);
    } else if (widget.ar.test!) {
      try {
        await FirebaseAuth.instance
          ...
          reference.doc(uid).set({
            'fullname': widget.ar.fullname,
            'phone': widget.ar.phone,
            'email': widget.ar.email,
            'gender': widget.ar.gender,
            'dc_type': widget.ar.dctype,
            'font_image': widget.ar.front,
            'Back_image': widget.ar.back,
            'uid': uid,
            'pro_image': images[0].imageUrl,
            'password': widget.ar.password
          }).then((value) {
            print("User Added");
